...
/**
 * GET /admin/employees
 * Fetches all companies and their linked employees
 */
router.get('/employees', authenticateToken, async (req: AuthRequest, res: Response, next: NextFunction): Promise<any> => {
  try {
    if (!req.user || req.user.role !== 'ADMIN') {
      return res.status(403).json({ message: 'Forbidden' });
    }
    ...
    const employees = await Employee.find().populate('userId', 'name email').lean();

    const result = companies.map(c => {
      const companyEmployees = employees.filter(e => e.companyId.toString() === c._id.toString());
      return {
        company: {
          _id: c._id,
          name: c.name,
 ...
 * Returns dynamic logs of invoices and payrolls generated by companies (aggregated by day)
 */
router.get('/usage-logs', authenticateToken, async (req: AuthRequest, res: Response, next: NextFunction): Promise<any> => {
  try {
    if (!req.user || req.user.role !== 'ADMIN') {
      return res.status(403).json({ message: 'Forbidden' });
    }
    ...
    const invLogs = await populateCompany(invoices, 'INVOICE');
    const prLogs = await populateCompany(payrolls, 'PAYROLL');

    const logs = [...invLogs, ...prLogs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
